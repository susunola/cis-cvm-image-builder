...
| Step | Lane | What it does |
|------|------|--------------|
| `ciscvm.toml` | CLI | Single source of truth: profile, cloud params, CIS level, audit dir |
| `preflight` | CLI | Validates creds, packer on PATH, required params, WinRM reachability (Windows) |
| `render` | CLI | Generates `main.pkr.hcl`, Ansible playbooks, install/verify scripts from templates |
| `packer build` | Packer | Launches a temporary CVM, runs the `tencentcloud-cvm` builder + provisioner chain |
| `ansible-local` | Ansible CIS | Linux: bootstraps Ansible inside the VM, applies `ansible-lockdown` CIS roles. Windows: WinRM + remote ansible (controller-side) |
| `goss audit` | Audit | Linux-only build-time gate; aborts the build if `cis_max_failures` is exceeded |
| Image Output | Output | Hardened CVM image, tagged with profile + level + timestamp, optionally copied to other regions |
...
├── docs/                        # Diagrams (see How it works)
│   ├── ciscvm-pipeline.html     #   Interactive version (theme toggle, export)
