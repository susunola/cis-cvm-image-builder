...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "# regardless of firewalld/nftables runtime state.  The permanent rules",
      "# written above normally survive, but on TencentOS 4 the apply playbook",
      "# can leave firewalld in a state where reboot drops the port again",
      "# (observed: permanent allow on ALL zones, yet post-reboot i/o timeout).",
      ...
      "    firewall-cmd --zone=$z --add-port=$SSH_PORT/tcp >/dev/null 2>&1 || true",
      "    firewall-cmd --zone=$z --add-port=$SSH_PORT/tcp --permanent >/dev/null 2>&1 || true",
      "  done",
      "  firewall-cmd --reload >/dev/null 2>&1 || true",
      "fi",
      "if command -v nft >/dev/null 2>&1; then",
      "  for t in $(nft list tables 2>/dev/null | awk '{print $2}'); do",
      "    nft add rule $t input tcp dport $SSH_PORT accept 2>/dev/null || true",
      "  done",
      "fi",
