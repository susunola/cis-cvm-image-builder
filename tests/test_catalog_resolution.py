...
def test_cis_maps_to_rules_json():
    ...
    for bm in ("", "CIS-v1.0.0", "cis", "CIS benchmark"):
        assert _catalog_basename("ohbs-rhel9", bm) == "rules.json"
        assert _catalog_path("ohbs-rhel9", bm).name == "rules.json"
...
def test_stig_resolves_to_specific_file_when_present(tmp_path, monkeypatch):
    ...
    files = tmp_path / "roles" / "ohbs-rhel9" / "files"
    ...
    assert _catalog_basename("ohbs-rhel9", "STIG-RHEL9") == "rules_stig_rhel9.json"
    assert _catalog_path("ohbs-rhel9", "STIG-RHEL9").name == "rules_stig_rhel9.json"
...
def test_stig_falls_back_to_rules_json_when_absent():
    ...
    assert _catalog_basename("ohbs-rhel9", "STIG-RHEL9") == "rules.json"
...
def test_workspace_rules_json_takes_precedence(tmp_path):
    ...
    files = workdir / "ansible" / "roles" / "ohbs-rhel9" / "files"
    files.mkdir(parents=True)
    (files / "rules.json").write_text('{"overrides": true}')

    resolved = _catalog_path("ohbs-rhel9", "STIG-RHEL9", workdir=workdir)
