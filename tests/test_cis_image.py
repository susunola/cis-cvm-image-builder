"""Tests for cis-image — CIS-hardened Golden Image Builder."""
...
def _suppress_logging(caplog: pytest.LogCaptureFixture) -> None:
    caplog.set_level(logging.DEBUG, logger="cis-image")
...
class TestResolve:
    ...
    def test_assume_role_default_off(self, valid_toml):
        ...
        assert r.assume_role_session == "cis-image"
...
class TestExtractImageIds:
    def test_multi_region_artifact(self):
        ...
        assert cis_image._extract_image_ids(lines) == ["img-1p9mwidq", "img-50m2n24g"]

    def test_single_region_artifact(self):
        lines = [
            "--> tencentcloud-cvm.default: Tencentcloud images(ap-guangzhou: img-abc123) were created.",
        ]
        assert cis_image._extract_image_ids(lines) == ["img-abc123"]
...
class TestRenderAll:
    def test_linux_renders_correctly(self, valid_toml, tmp_path):
        ...
        assert (wd / "packer" / "scripts" / "cis-image-finalize.sh").exists()
        assert os.access(wd / "packer" / "scripts" / "cis-image-finalize.sh", os.X_OK)
    ...
    def test_banner_and_report_provisioner_present(self, valid_toml, tmp_path):
        ...
        step that writes /etc/cis-image/banner, /etc/motd, and /opt/cis-image-REPORT.md."""
        r = resolve(valid_toml)
        wd = tmp_path / "build"
        render_all(wd, r)
        hcl = (wd / "packer" / "main.pkr.hcl").read_text()
        finalize = (wd / "packer" / "scripts" / "cis-image-finalize.sh").read_text()
        ...
        assert "cis-image-AUDIT-RESULT.json" in hcl
        assert "collect-audit.sh" in hcl
        assert "cis-image-finalize.sh" in hcl
        assert 'source      = "packer/scripts/cis-image-finalize.sh"' in hcl
        assert 'destination = "/opt/cis-image-ansible/cis-image-finalize.sh"' in hcl
        assert 'remote_path  = "/root/cis-image-run-finalize.sh"' in hcl
        assert "run-finalize.sh" in hcl
        # Finalize script: all the in-image channels are written.
        assert "/etc/cis-image/banner" in finalize
        assert "/etc/motd" in finalize
        assert "/etc/issue" in finalize
        assert "/etc/issue.net" in finalize
        assert "/etc/ssh/sshd_config.d/99-cis-image-banner.conf" in finalize
        assert "/opt/cis-image-REPORT.md" in finalize
        assert "/usr/local/bin/cis-image-info" in finalize
        assert "Banner /etc/cis-image/banner" in finalize
        # The source image, OS and cis-image version are wired through.
        ...
        import subprocess
        p = subprocess.run(
            ["bash", "-n", str(wd / "packer" / "scripts" / "cis-image-finalize.sh")],
    ...
    def test_windows_has_no_banner_provisioner(self, tmp_path):
        ...
        render_all(wd, r)
        hcl = (wd / "packer" / "main.pkr.hcl").read_text()
        assert "cis-image-finalize.sh" not in hcl
        assert "cis-image-AUDIT-RESULT.json" not in hcl
        assert not (wd / "packer" / "scripts" / "cis-image-finalize.sh").exists()

    def test_report_generator_handles_audit_json(self, valid_toml, tmp_path):
        """The python heredoc embedded in cis-image-finalize.sh must produce a
        ...
        for ln in finalize.splitlines():
            if ln.startswith("sudo /opt/cis-image-ansible/bin/python"):
                in_py = True
                continue
        ...
        report_p = tmp_path / "cis-image-REPORT.md"
        ...
        # Headings + key facts.
        assert "# cis-image — CIS Hardening Report" in body
...
class TestPackaging:
    ...
    Regression: roles/ must live *inside* the cis-image package (next to
    __init__.py), otherwise wheels omit them and `cis-image build` fails after
    a clean install.
    """

    def test_roles_dir_inside_package(self):
        pkg_dir = Path(cis_image.__file__).parent
...
class TestCmdInit:
    def test_creates_config(self, tmp_path):
        ...
        assert rc == 0
        assert (tmp_path / "cis-image.toml").exists()
        assert (tmp_path / ".gitignore").exists()

    def test_refuses_overwrite_without_force(self, tmp_path):
        (tmp_path / "cis-image.toml").write_text("existing", encoding="utf-8")
        rc = cmd_init(mock.MagicMock(target=str(tmp_path), force=False))
        assert rc == 1
...
class TestCmdClean:
    ...
    def test_not_a_cis_image_dir(self, tmp_path):
        """Refuse to clean a directory without cis-image markers."""
        wd = tmp_path / "not-cis-image"
        wd.mkdir()
        rc = cmd_clean(mock.MagicMock(workdir=str(wd)))
        assert rc == 1
...
class TestCmdBuildOutput:
    ...
    def test_build_does_not_reprint_output(self, tmp_path, capsys):
        from cis_image import PackerResult, cmd_build

        r, wd = self._prep(tmp_path)
        packer_lines = ["==> building", "Created image ID: img-xyz789", "done"]
        with (
            mock.patch("cis_image._load_resolve_preflight", return_value=(r, wd)),
            mock.patch("cis_image.render_all"),
            mock.patch(
                "cis_image.run_packer",
    ...
    def test_build_quiet_does_not_dump_log(self, tmp_path, capsys):
        ...
        with (
            mock.patch("cis_image._load_resolve_preflight", return_value=(r, wd)),
            mock.patch("cis_image.render_all"),
            mock.patch("cis_image.run_packer", side_effect=fake_run_packer),
        ):
    ...
    def test_build_aborts_when_preflight_fails(self, tmp_path):
        ...
        from cis_image import cmd_build

        with (
            mock.patch("cis_image._load_resolve_preflight", return_value=None),
            mock.patch("cis_image.render_all") as mock_render,
            mock.patch("cis_image.run_packer") as mock_run,
        ):
...
class TestCmdValidateOutput:
    ...
    def test_validate_renders_and_invokes_packer(self, valid_toml, tmp_path, monkeypatch):
        from cis_image import PackerResult, cmd_validate
    ...
    def test_validate_propagates_failure(self, valid_toml, tmp_path, monkeypatch):
        ...
        with (
            mock.patch("shutil.which", return_value="/usr/bin/packer"),
            mock.patch("cis_image.run_packer", return_value=PackerResult(exit_code=3)),
        ):
            rc = cmd_validate(
                mock.MagicMock(config=str(cfg), workdir=str(wd), quiet=False)
            )
    ...
    def test_validate_aborts_when_preflight_fails(self, tmp_path):
        from cis_image import cmd_validate
...
class TestCleanIsSafe:
    def test_allows_valid_cis_image_dir(self, tmp_path):
        wd = tmp_path / "build"
...
class TestColor:
    ...
    def test_non_tty_stderr(self, monkeypatch):
        ...
        with mock.patch.object(cis_image.sys.stderr, "isatty", return_value=False):
            result = _color("hello", 31)
            assert "\033" not in result

    def test_tty_produces_ansi(self, monkeypatch):
        monkeypatch.delenv("NO_COLOR", raising=False)
        with mock.patch.object(cis_image.sys.stderr, "isatty", return_value=True):
            result = _color("hello", 31)
...
class TestAllProfilesRender:
    ...
    def test_ssh_guard_nft_awk_and_reconnect_budget(self, profile_name, valid_toml, tmp_path):
        ...
        if r.family != "windows":
            ...
            assert f'remote_path       = "{remote_dir}/cis-image-reconnected.sh"' in hcl, (
...
class TestBuildGovernance:
    ...
    def test_lineage_record_and_images(self, valid_toml, tmp_path):
        ...
        with mock.patch("cis_image._lineage_path", return_value=lin):
            from cis_image import _record_lineage, cmd_images
    ...
    def test_provenance_unsigned_when_no_key(self, valid_toml, tmp_path):
        from cis_image import _write_provenance
        r = resolve(valid_toml)
        with mock.patch("cis_image._lineage_path", return_value=tmp_path / "lineage.jsonl"):
...
class TestVerify:
    """cis-image verify — SLSA provenance signature verification."""

    def _make_prov(self, tmp_path, image_id="img-abc", signed=True, key="TESTKEY"):
        import cis_image
        from cis_image import SAMPLE_CONFIG, _write_provenance
        cis_image._lineage_path = lambda: tmp_path / "lineage.jsonl"
    ...
    def test_verify_valid_signature(self, valid_toml, tmp_path):
        from cis_image import cmd_verify
...
class TestScanListRules:
    """cis-image scan / list / [cis].rules_include|exclude (roadmap v0.14.3)."""
    ...
    def test_cmd_list_output(self, capsys):
        from cis_image import cmd_list
    ...
    def test_cmd_scan_gate_fail(self, valid_toml, tmp_path):
        from cis_image import PackerResult, cmd_scan
        r = resolve(valid_toml)
        with (
            mock.patch("cis_image._load_resolve_preflight", return_value=(r, tmp_path / "b")),
            mock.patch("cis_image.render_all"),
            mock.patch("cis_image.run_packer",
                       return_value=PackerResult(exit_code=0, stdout_lines=[
                           "Tencentcloud images(ap-guangzhou: img-scan1) were created.",
                           "Score: 80.0%"])),
            mock.patch("cis_image._record_lineage") as lin,
        ):
        ...
        with (
            ...
            mock.patch("cis_image.run_packer",
                       return_value=PackerResult(exit_code=0, stdout_lines=[
                           ...
                           "Score: 92.0%"])),
            mock.patch("cis_image._record_lineage") as lin,
            mock.patch("cis_image._write_provenance") as prov,
        ):
            rc = cmd_scan(mock.MagicMock(config="x", workdir="b", yes=True, quiet=True,
                                         debug=False, min_score=85.0))
        assert rc == 0
...
class TestCleanupImages:
    """cis-image cleanup-images — retire old images by lineage age."""
    ...
    def test_dry_run_no_delete(self, tmp_path):
        ...
        with mock.patch("cis_image._delete_images") as dele, \
             mock.patch("cis_image._images_exist") as exist:
    ...
    def test_apply_deletes_and_marks_retired(self, tmp_path):
        ...
        with mock.patch("cis_image._images_exist", return_value=["img-old1", "img-old2", "img-old3"]), \
             mock.patch("cis_image._delete_images") as dele:
    ...
    def test_tc3_signer_shape(self, tmp_path):
        ...
        with mock.patch("cis_image.urllib.request.urlopen", side_effect=fake_urlopen):
            ...
            with mock.patch.dict(_os.environ, {"TENCENTCLOUD_SECRET_ID": "AKIDtest", "TENCENTCLOUD_SECRET_KEY": "sk-test"}):
                out = cis_image._images_exist("ap-guangzhou", ["img-x"])
        assert out == ["img-x"]
...
class TestIdempotencyAndSarif:
    ...
    def test_cmd_test_pass(self, valid_toml, tmp_path):
        from cis_image import PackerResult, cmd_test
        r = resolve(valid_toml)
    ...
    def test_sarif_build(self):
        from cis_image import _build_sarif
        out = _build_sarif([
            "==> something",
        ...
        run = d["runs"][0]
        assert run["tool"]["driver"]["name"] == "cis-image"
    ...
    def test_scan_sarif_written(self, valid_toml, tmp_path):
        ...
        with (
            ...
            mock.patch("cis_image._record_lineage"),
            mock.patch("cis_image._write_provenance"),
        ):
    ...
    def test_xccdf_parses_ansible_msg_wrapped_output(self):
        from cis_image import _build_xccdf
        out = _build_xccdf(["Score:     60.0%", self.MSG_WRAPPED])
        assert 'idref="xccdf_org.cis_image.content_rule_1.1.1.1"' in out
        assert 'idref="xccdf_org.cis_image.content_rule_1.5.1"' in out
...
class TestAuditRuleMatching:
    ...
    def _engine():
        ...
        spec = importlib.util.spec_from_file_location(
            "cis_engine",
            "cis_image/roles/cis_tencentos4/files/cis_engine.py")
...
class TestAuditDedup:
    ...
    def test_4_1_3_24_key_not_privileged(self):
        import json
        d = json.load(open("cis_image/roles/cis_tencentos4/files/rules.json"))
    ...
    def test_engine_cross_file_dedup(self):
        ...
        with open("cis_image/roles/cis_tencentos4/files/cis_engine.py", encoding="utf-8") as fh:
            src = fh.read()
...
class TestRuleOverrides:
    ...
    def test_apply_merges_params_into_workspace_copy(self, valid_toml, tmp_path):
        ...
        with open("cis_image/roles/cis_tencentos3/files/rules.json", encoding="utf-8") as fh:
...
class TestFingerprintAndChangeDetection:
    ...
    def test_fingerprint_deterministic(self, valid_toml):
        from cis_image import _build_fingerprint
    ...
    def test_fingerprint_changes_with_rules_hash(self, valid_toml, monkeypatch):
        from cis_image import _build_fingerprint
        r = resolve(valid_toml)
        monkeypatch.setattr("cis_image._bundled_rules_hash", lambda rd: "0" * 64)
        fp1 = _build_fingerprint(r)
        monkeypatch.setattr("cis_image._bundled_rules_hash", lambda rd: "1" * 64)
    ...
    def test_pending_skips_when_unchanged(self, valid_toml, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("cis_image._image_ids_still_exist", lambda r, ids: True)
    ...
    def test_pending_requires_rebuild_when_changed(self, valid_toml, tmp_path, monkeypatch):
        from cis_image import cmd_pending, resolve
        r = resolve(valid_toml)
        line = {
            "ts": "2026-08-01T00:00:00Z", "status": "ok",
            "profile": r.profile_name, "cis_level": r.level,
...
class TestCveScanAndSbom:
    ...
    def test_sbom_block_rendered(self, valid_toml, tmp_path):
        ...
        render_all(wd, r)
        hcl = (wd / "packer" / "main.pkr.hcl").read_text(encoding="utf-8")
        assert "cis-image-sbom.sh" in hcl
        assert "SBOM_SHA256" in hcl
        assert "cis-image-SBOM.jsonl" in hcl
    ...
    def test_supply_chain_absent_by_default(self, valid_toml, tmp_path):
        ...
        render_all(wd, r)
        hcl = (wd / "packer" / "main.pkr.hcl").read_text(encoding="utf-8")
        assert "cis-image-cve-scan.sh" not in hcl
        assert "cis-image-sbom.sh" not in hcl
        assert "__SUPPLY_CHAIN_BLOCK__" not in hcl

    def test_sbom_sha_and_count_extraction(self):
        from cis_image import _extract_sbom_count, _extract_sbom_sha
        lines = [
            "[cis-image] sbom: 137 packages -> /opt/cis-image-SBOM.jsonl",
            "[cis-image] SBOM_SHA256=" + "a" * 64,
        ]
...
class TestShareImages:
    ...
    def test_share_calls_api_with_accounts(self, valid_toml, monkeypatch):
        from cis_image import _share_images, resolve
        calls = {}
...
class TestBuildReportArchive:
    ...
    ~/.cis-image/reports/<image>.json — Linux via a gzipped+base64 marker line
    in the packer log, Windows via the role-fetched result.json."""

    def test_extract_from_linux_marker(self, tmp_path, monkeypatch):
        import base64, gzip
        from cis_image import _save_build_report
        monkeypatch.setattr("cis_image._reports_dir", lambda: tmp_path)
    ...
    def test_extract_from_windows_fetched_result(self, tmp_path, monkeypatch):
        from cis_image import _save_build_report
        monkeypatch.setattr("cis_image._reports_dir", lambda: tmp_path / "out")
    ...
    def test_linux_hcl_emits_marker(self):
        from cis_image import HCL_LINUX_TEMPLATE
...
class TestWindowsShipAuditResult:
    ...
    /opt/cis-image-AUDIT-RESULT.json — so report/drift tooling works without
    re-running the engine."""

    def test_site_template_sets_ship_path(self):
        from cis_image import SITE_YML_WIN_TEMPLATE
    ...
    def test_all_windows_roles_support_ship_result(self):
        import glob
        for run_yml in glob.glob("cis_image/roles/cis_win*/tasks/run.yml"):
            content = open(run_yml, encoding="utf-8").read()
            assert "cis_ship_result_path" in content, run_yml
        for defaults in glob.glob("cis_image/roles/cis_win*/defaults/main.yml"):
...
class TestIndependentAudit:
    """P0#1 — cis-image audit (oscap / inspec) + parsers."""

    def test_parse_oscap_arf(self):
        from cis_image import _parse_oscap_arf
        xml = """<?xml version="1.0"?>
<arf xmlns="http://scap.nist.gov/schema/asset-reporting-format/1.1">
  <report>
    ...
    def test_parse_inspec_json(self):
        from cis_image import _parse_inspec_json
        data = {"controls": [
    ...
    def test_audit_oscap_requires_datastream(self):
        from cis_image import cmd_audit
...
class TestKittyCsvParser:
    ...
    def test_parse_basic(self):
        from cis_image import _parse_kitty_csv
...
class TestRuleIdAndBenchmark:
    ...
    def test_all_linux_engines_in_sync(self):
        ...
        for role in ("cis_tencentos4", "cis_tencentos3", "cis_rhel8",
            ...
            with open(f"cis_image/roles/{role}/files/cis_engine.py", "rb") as fh:
                data = fh.read()
    ...
    def test_none_risk_rules_never_applied(self):
        ...
        with open("cis_image/roles/cis_ubuntu2404/files/cis_engine.py", encoding="utf-8") as fh:
            src = fh.read()
    ...
    def test_none_risk_partition_rules_are_manual(self):
        ...
        import glob as _g
        for path in _g.glob("cis_image/roles/cis_*/files/rules.json"):
...
class TestVerifyImage:
    ...
    def test_probe_launch(self, valid_toml, monkeypatch):
        ...
        assert _probe_launch(r, "img-new", "cis-image-verify") == "ins-probe"
    ...
    def test_probe_terminate(self, valid_toml, monkeypatch):
        from cis_image import _probe_terminate
        r = resolve(valid_toml)
        monkeypatch.setenv("TENCENTCLOUD_SECRET_ID", "AKIDx")
        monkeypatch.setenv("TENCENTCLOUD_SECRET_KEY", "key")
        called = []
    ...
    def test_verify_image_success_path(self, valid_toml, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("cis_image._load_resolve_preflight",
                            lambda c, w: (r, tmp_path / "w"))
        monkeypatch.setattr("cis_image._probe_launch", lambda *a, **k: "ins-probe")
        monkeypatch.setattr("cis_image._probe_public_ip", lambda *a, **k: "1.2.3.4")
        monkeypatch.setattr("cis_image._probe_ssh_ready", lambda *a, **k: True)
        monkeypatch.setattr("cis_image._probe_scan",
                            lambda *a, **k: {"summary": {"all": {"score": 96.0, "fail": 0}}})
        terminated = []
        monkeypatch.setattr("cis_image._probe_terminate",
...
class TestBuildNewFeatures:
    ...
    def test_build_skips_when_unchanged(self, tmp_path, monkeypatch):
        from cis_image import PackerResult, cmd_build
        r, wd = self._prep(tmp_path)
        monkeypatch.setattr("cis_image._load_resolve_preflight", lambda c, w: (r, wd))
        monkeypatch.setattr("cis_image._build_fingerprint", lambda r_: "fp123")
        monkeypatch.setattr("cis_image._last_successful_fingerprint",
                            lambda r_: ("fp123", ["img-old"]))
        monkeypatch.setattr("cis_image._image_ids_still_exist", lambda r_, ids: True)
        rendered = []
        monkeypatch.setattr("cis_image.render_all", lambda w, r: rendered.append(1))
        run = []
        monkeypatch.setattr("cis_image.run_packer", lambda *a, **k: run.append(1) or
    ...
    def test_build_verify_boot_wires_probe(self, tmp_path, monkeypatch, caplog):
        ...
        monkeypatch.setattr("cis_image._load_resolve_preflight", lambda c, w: (r, wd))
        monkeypatch.setattr("cis_image.render_all", lambda w, r: None)
        monkeypatch.setattr("cis_image.run_packer", lambda *a, **k:
                            PackerResult(exit_code=0, stdout_lines=[
                                "Created image ID: img-new"]))
        monkeypatch.setattr("cis_image.cmd_verify_image", lambda a, image_id=None: 0)
        args = mock.MagicMock(config="x", workdir=str(wd), yes=True, quiet=False,
    ...
    def test_build_verify_boot_fail_blocks_image(self, tmp_path, monkeypatch, caplog):
        ...
        monkeypatch.setattr("cis_image.run_packer", lambda *a, **k:
                            PackerResult(exit_code=0, stdout_lines=[
                                "Created image ID: img-new"]))
        monkeypatch.setattr("cis_image.cmd_verify_image", lambda a, image_id=None: 1)
        args = mock.MagicMock(config="x", workdir=str(wd), yes=True, quiet=False,
                              log_file=None, skip_if_unchanged=False)
    ...
    def test_build_share_wired(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("cis_image.run_packer", lambda *a, **k:
                            PackerResult(exit_code=0, stdout_lines=[
                                "Created image ID: img-new"]))
        shared = []
        monkeypatch.setattr("cis_image._share_images",
    ...
    def test_build_captures_sbom(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("cis_image.run_packer", lambda *a, **k:
                            PackerResult(exit_code=0, stdout_lines=[
                                "Created image ID: img-new",
                                "[cis-image] sbom: 42 packages -> /opt/cis-image-SBOM.jsonl",
                                "[cis-image] SBOM_SHA256=" + "b" * 64,
                            ]))
        lineage = {}
        monkeypatch.setattr("cis_image._record_lineage",
                            lambda r_, ids, name, score, ok, sbom_sha=None,
                            sbom_count=None: lineage.update(
                                {"sha": sbom_sha, "count": sbom_count}) or None)
        prov = {}
        monkeypatch.setattr("cis_image._write_provenance",
...
class TestProvenanceSbom:
    ...
    def test_provenance_records_sbom(self, valid_toml, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("cis_image._lineage_path",
                            lambda: home / ".cis-image" / "lineage.jsonl")
        monkeypatch.setattr("cis_image._bundled_rules_hash", lambda rd: "r" * 64)
        monkeypatch.setattr("cis_image._build_fingerprint", lambda r_: "f" * 64)
...
class TestLineageFields:
    ...
    def test_lineage_records_benchmark_and_fingerprint(self, valid_toml,
        ...
        from cis_image import _record_lineage
        r = resolve(valid_toml)
        home = tmp_path / "home"
        monkeypatch.setattr("cis_image._lineage_path",
                            lambda: home / ".cis-image" / "lineage.jsonl")
        monkeypatch.setattr("cis_image._build_fingerprint", lambda r_: "fp-1")
...
class TestTestComponents:
    ...
    def test_rendered_with_uploads(self, valid_toml, tmp_path):
        ...
        render_all(wd, r)
        hcl = (wd / "packer" / "main.pkr.hcl").read_text(encoding="utf-8")
        assert "cis-image-user-tests.sh" in hcl
    ...
    def test_not_rendered_when_unset(self, valid_toml, tmp_path):
        ...
        render_all(wd, r)
        hcl = (wd / "packer" / "main.pkr.hcl").read_text(encoding="utf-8")
        assert "cis-image-user-tests.sh" not in hcl
...
class TestDeployWebhook:
    ...
    def test_trigger_on_success(self, valid_toml, monkeypatch):
        from cis_image import _trigger_deploy_webhook, resolve
        r = resolve(valid_toml)
        ...
        monkeypatch.setattr("cis_image.urllib.request.urlopen", fake_open)
    ...
    def test_no_trigger_when_webhook_empty(self, valid_toml, monkeypatch):
        from cis_image import _send_notification, resolve
        r = resolve(valid_toml)
        r.deploy_webhook = ""
        called = []
        monkeypatch.setattr("cis_image._trigger_deploy_webhook",
    ...
    def test_trigger_wired_from_notification(self, valid_toml, monkeypatch):
        ...
        monkeypatch.setattr("cis_image.urllib.request.urlopen",
...
class TestDrift:
    ...
    def test_drift_diff_new_failures(self):
        from cis_image import _drift_diff
...
class TestUnusedSince:
    ...
    def test_shared_images_kept(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("cis_image._lineage_path",
                            lambda: home / ".cis-image" / "lineage.jsonl")
        monkeypatch.setattr("cis_image._image_is_shared", lambda region, img: True)
        monkeypatch.setattr("cis_image._images_exist", lambda r, ids: ids)
        monkeypatch.setattr("cis_image._delete_images", lambda r, ids: None)
    ...
    def test_unshared_images_deleted(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("cis_image._lineage_path",
                            lambda: home / ".cis-image" / "lineage.jsonl")
        monkeypatch.setattr("cis_image._image_is_shared", lambda region, img: False)
        monkeypatch.setattr("cis_image._images_exist", lambda r, ids: ids)
        deleted = []
        monkeypatch.setattr("cis_image._delete_images",
    ...
    def test_image_is_shared_api(self, monkeypatch):
        from cis_image import _image_is_shared
        monkeypatch.setenv("TENCENTCLOUD_SECRET_ID", "AKIDx")
        monkeypatch.setenv("TENCENTCLOUD_SECRET_KEY", "key")
        monkeypatch.setattr(
            "cis_image._tc3_api",
...
class TestCheckSource:
    ...
    def test_source_created_recorded_in_lineage(self, valid_toml, tmp_path, monkeypatch):
        from cis_image import _record_lineage, resolve
        r = resolve(valid_toml)
        home = tmp_path / "home"
        monkeypatch.setattr("cis_image._lineage_path",
                            lambda: home / ".cis-image" / "lineage.jsonl")
        monkeypatch.setattr("cis_image._build_fingerprint", lambda r_: "fp")
        monkeypatch.setattr("cis_image._source_image_created",
    ...
    def test_check_source_unchanged(self, valid_toml, tmp_path, monkeypatch):
        from cis_image import cmd_check_source, resolve
        r = resolve(valid_toml)
        home = tmp_path / "home"
        (home / ".cis-image").mkdir(parents=True)
        (home / ".cis-image" / "lineage.jsonl").write_text(
...
class TestListVersions:
    """#19 — cis-image list --versions shows rule-catalog hash."""
...
class TestTestComponentsNonRoot:
    ...
    def test_ubuntu_renders_home_destination(self, valid_toml, tmp_path):
        ...
        render_all(wd, r)
        hcl = (wd / "packer" / "main.pkr.hcl").read_text(encoding="utf-8")
        assert "/home/ubuntu/cis-image-test-components/00-check.sh" in hcl
        assert "/root/cis-image-test-components/" not in hcl
        # runner loop resolves __REMOTE_DIR__ to /home/ubuntu in the rendered HCL
        assert "for t in /home/ubuntu/cis-image-test-components/*" in hcl
    ...
    def test_runner_template_has_no_hardcoded_root(self):
        ...
        assert "for t in /root/cis-image-test-components/*" not in src
...
class TestVerifyImageMinScoreFallback:
    ...
    def test_fallback_uses_config_min_score(self, valid_toml, monkeypatch, tmp_path):
        ...
        r = resolve(valid_toml)
        monkeypatch.setattr("cis_image._load_resolve_preflight", lambda c, w: (r, tmp_path / "w"))
        monkeypatch.setattr("cis_image._probe_launch", lambda *a, **k: "ins-probe")
        monkeypatch.setattr("cis_image._probe_public_ip", lambda *a, **k: "1.2.3.4")
        monkeypatch.setattr("cis_image._probe_ssh_ready", lambda *a, **k: True)
        monkeypatch.setattr("cis_image._probe_scan",
                            lambda *a, **k: {"summary": {"all": {"score": 90.0, "fail": 2}}})
        monkeypatch.setattr("cis_image._probe_terminate", lambda *a, **k: None)
    ...
    def test_probe_launch_failure_returns_cleanly(self, valid_toml, monkeypatch, tmp_path):
        ...
        r = resolve(valid_toml)
        monkeypatch.setattr("cis_image._load_resolve_preflight", lambda c, w: (r, tmp_path / "w"))
        monkeypatch.setattr("cis_image._probe_launch",
                            lambda *a, **k: (_ for _ in ()).throw(ConfigError("no creds")))
        terminated = []
        monkeypatch.setattr("cis_image._probe_terminate", lambda r_, i: terminated.append(i))
        args = mock.MagicMock(config="c", workdir="w", image="img-new", min_score=85.0)
...
class TestProbeScanTimeout:
    ...
    def test_timeout_returns_error_dict(self, valid_toml, monkeypatch):
        from cis_image import _probe_scan
        r = resolve(valid_toml)
...
class TestCleanupRetiredGranularity:
    ...
    def test_partial_delete_keeps_survivor_active(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("cis_image._lineage_path",
                            lambda: home / ".cis-image" / "lineage.jsonl")
        monkeypatch.setattr("cis_image._images_exist", lambda region, ids: ids)
        deleted = []
        monkeypatch.setattr("cis_image._delete_images", lambda r, ids: deleted.extend(ids))
...
class TestEnginePy38Compat:
    ...
    ENGINES = sorted(glob.glob("cis_image/roles/*/files/cis_engine.py"))
...
class TestLinuxRulePolicyConsistency:
    ...
    CATALOGS = sorted(glob.glob("cis_image/roles/cis_*/files/rules.json"))
