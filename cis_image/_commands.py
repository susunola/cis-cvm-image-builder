...
def cmd_preflight(args: argparse.Namespace) -> int:
    ...
    result = cis_image._load_resolve_preflight(args.config, args.workdir)
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    cis_image.render_all(workdir, r)

    banner("validate")
    info(f"Rendered working directory: {workdir}")
    info("Running packer init + packer validate ...")
    result = cis_image.run_packer(workdir, "validate", quiet=args.quiet, debug=args.debug)
...
def cmd_build(args: argparse.Namespace) -> int:
    ...
    if success:
        ...
        lin = cis_image._record_lineage(r, image_ids, image_name, score, ok=True,
                              sbom_sha=sbom_sha, sbom_count=sbom_count)
        if lin:
            info(f"Lineage recorded -> {lin}")
        prov = cis_image._write_provenance(r, image_ids, image_name, score,
                                 sbom_sha=sbom_sha, sbom_count=sbom_count)
        if prov:
        ...
        if share_targets and image_ids:
            cis_image._share_images(r, image_ids, share_targets)
        ...
        if r.verify_boot and image_ids:
            if r.family == "windows":
                ...
                ok("Clean-boot verification: booting probe instance from "
                   f"{image_ids[0]} …")
                vrc = cis_image.cmd_verify_image(args, image_id=image_ids[0])
                if vrc != 0:
...
def cmd_check_source(args: argparse.Namespace) -> int:
    ...
    if prep is None:
        return 1
    r, _workdir = prep
    now_created = cis_image._source_image_created(r)
    if not now_created:
...
def cmd_verify_image(args: argparse.Namespace, image_id: str | None = None) -> int:
    ...
    try:
        try:
            instance_id = cis_image._probe_launch(r, image_id, f"cis-image-verify-{image_id}")
        except ConfigError as exc:
            fail(str(exc))
            return 1
        ok(f"Probe instance: {instance_id}")
        ip = cis_image._probe_public_ip(r, instance_id)
        if not ip:
        ...
        if not cis_image._probe_ssh_ready(ip, ssh_port, ssh_user):
            fail("SSH did not come up on the probe instance (timeout) — "
                 "clean-boot verification failed")
            return 1
        ok("SSH ready on fresh boot")
        doc = cis_image._probe_scan(r, ip, ssh_port, ssh_user, r.level)
...
def cmd_save_baseline(args: argparse.Namespace) -> int:
    ...
    ssh_user = getattr(args, "ssh_user", "") or r.ssh_username or "root"
    ssh_port = int(getattr(args, "ssh_port", 0) or r.ssh_port or 22)
    doc = cis_image._probe_scan(r, host, ssh_port, ssh_user, r.level)
    if "error" in doc and "summary" not in doc:
    ...
    out = cis_image._lineage_path().parent / "baselines" / f"{safe}.json"
    ...
    *--unused-since N* (#16): additionally require the image to be
    ...
    if unused_since:
        ...
        for rec, img in candidates:
            if cis_image._image_is_shared(rec.get("region", ""), img):
                kept_shared += 1
    ...
    for region, imgs in sorted(by_region.items()):
        for img in sorted(set(imgs)):
            if args.apply:
                try:
                    existing = cis_image._images_exist(region, [img])
                    if not existing:
...
def cmd_test(args: argparse.Namespace) -> int:
    ...
    cis_image.render_all(workdir, r, idempotency=args.idempotency)
    banner("test")
...
def cmd_pending(args: argparse.Namespace) -> int:
    ...
    if prep is None:
        return 1
    r, _workdir = prep
    fp = cis_image._build_fingerprint(r)
    prev_fp, prev_images = cis_image._last_successful_fingerprint(r)
    ...
    *--versions* (#19): additionally show the bundled rule-catalog sha256
    ...
    if show_versions:
        ...
        for name, meta in sorted(PROFILES.items()):
            ...
            rh = cis_image._bundled_rules_hash(role)[:16] if role else "-"
...
def cmd_scan(args: argparse.Namespace) -> int:
    ...
    *evaluates* the rules — nothing is modified.  The final re-audit score
    ...
    cis_image.render_all(workdir, r, scan=True)
    banner("scan")
    ...
    if image_ids:
        ok(f"Output image ID(s): {', '.join(image_ids)}")
    cis_image._record_lineage(r, image_ids, image_name, score, ok=True)
    cis_image._write_provenance(r, image_ids, image_name, score)
    rep = _save_build_report(r, image_name, result.stdout_lines, workdir)
    if rep:
        info(f"Audit report archived -> {rep}")
    ...
    if args.provenance:
        ...
        if not paths:
            fail(f"No provenance found for image {args.image} in "
                 f"{cis_image._lineage_path().parent / 'provenance'}")
            return 1
    else:
