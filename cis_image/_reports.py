...
def _save_build_report(r: ResolvedConfig, image_name: str,
    ...
    try:
        ...
        return None
    try:
        out = cis_image._reports_dir() / f"{image_name}.json"
...
def _build_fingerprint(r: ResolvedConfig) -> str:
    ...
    parts = [
        ...
        "benchmark", r.image_benchmark,
        "os", r.image_os_tag,
        "rules", cis_image._bundled_rules_hash(r.role_dir),
...
def _send_notification(r: ResolvedConfig, ok: bool, image_ids: list[str],
    ...
    if ok and r.deploy_webhook:
        cis_image._trigger_deploy_webhook(r, image_ids, score, image_name)
...
def _write_provenance(r: ResolvedConfig, image_ids: list[str], image_name: str,
    ...
    try:
        from datetime import datetime
        ts = datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
        dirp = cis_image._lineage_path().parent / "provenance"
            ...
            "predicate": {
                "buildDefinition": {
                    ...
                    "externalParameters": {
                        ...
                        "rules_sha256": cis_image._bundled_rules_hash(r.role_dir),
                        "fingerprint": cis_image._build_fingerprint(r),
                    },
