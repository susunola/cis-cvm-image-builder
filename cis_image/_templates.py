...
CVE_SCAN_LINUX_BLOCK = r"""  provisioner "shell" {
    ...
    inline = [
      ...
      "  echo '[cis-image] cve-scan: installing Trivy v0.74.0'",
      "  TARCH=$(uname -m | sed -e 's/x86_64/64bit/' -e 's/aarch64/ARM64/' -e 's/arm64/ARM64/')",
      "  case \"$TARCH\" in 64bit) TRIVY_SHA=2ae6fe3ee734b7fdf11335663e18c75ea12dccc76062f09f164a3b0f8be4371a ;; ARM64) TRIVY_SHA=b94ce1976bbf3c15b514b605ee88be7c6d94a29be2302847ff01cb794d47aad5 ;; *) echo '[cis-image] cve-scan: unsupported architecture'; exit 1 ;; esac",
      "  curl -fsSL \"https://github.com/aquasecurity/trivy/releases/download/v0.74.0/trivy_0.74.0_Linux-${TARCH}.tar.gz\" -o /tmp/trivy.tgz || { echo '[cis-image] cve-scan: Trivy download failed'; exit 1; }",
      "  echo \"$TRIVY_SHA  /tmp/trivy.tgz\" | sha256sum -c - || { echo '[cis-image] cve-scan: checksum verification failed'; exit 1; }",
      "  sudo tar -C /usr/local/bin -xzf /tmp/trivy.tgz trivy && rm -f /tmp/trivy.tgz || { echo '[cis-image] cve-scan: Trivy install failed'; exit 1; }",
      "fi",
      "if ! command -v trivy >/dev/null 2>&1; then",
      "  echo '[cis-image] cve-scan: Trivy unavailable — failing closed'",
      "  exit 1",
      "else",
